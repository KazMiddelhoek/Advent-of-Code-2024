...
fn part_one() {
    ...
    for (line_idx, line) in word_search_array.iter().enumerate() {
        for (letter_idx, letter) in line.iter().enumerate() {
            if *letter == 'X' {
                let options =
                    look_around_for_M(&word_search_array, (line_idx as isize, letter_idx as isize));
                for option in options {
                    if line_idx as isize + (2 * option.0) < 0
                        || line_idx as isize + (3 * option.0) < 0
                    {
                        continue;
                    }

                    if line_idx as isize + (2 * option.0) >= word_search_array.len() as isize
                        || line_idx as isize + (3 * option.0) >= word_search_array.len() as isize
                    {
                        continue;
                    }
                    if letter_idx as isize + (2 * option.1) < 0
                        || letter_idx as isize + (3 * option.1) < 0
                    {
                        continue;
                    }

                    if letter_idx as isize + (2 * option.1) >= word_search_array.len() as isize
                        || letter_idx as isize + (3 * option.1) >= word_search_array.len() as isize
                    {
                        continue;
                    }
                    if word_search_array[(line_idx as isize + (2 * option.0)) as usize]
                        [(letter_idx as isize + (2 * option.1)) as usize]
                        == 'A'
                    {
                        if word_search_array[(line_idx as isize + (3 * option.0)) as usize]
                            [(letter_idx as isize + (3 * option.1)) as usize]
                            == 'S'
                        {
                            n_xmas += 1
                        }
